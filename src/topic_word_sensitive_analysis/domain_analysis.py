...
OUTPUT_DIR = PROJECT_ROOT / "output" / "topic_analysis_sensitive" / "domain" 
DATA_PATH = PROJECT_ROOT / "output" / "topic_analysis_sensitive" / "domain" / "general_union_wordset_domain.json"
...
class DomainAnalyzer:
    ...
    def _create_general_union_wordset(self):
        ...
        try:
            # 直接读取现有的JSON文件
            general_union_path = self.output_dir / "general_union_wordset_domain.json"
            ...
            print("警告: 未找到general_union_wordset_domain.json文件，将创建新的")
            # 如果文件不存在，则创建新的
            general_union = {}
