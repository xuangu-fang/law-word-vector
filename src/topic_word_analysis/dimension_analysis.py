...
# -*- coding: utf-8 -*-
"""
Topic Analysis - 通用维度分析器

功能：
1. 通用的topic相似度分析框架
2. 支持灵活的era-keyword组合配置
3. 提供多种分析模式（union、general_union、归一化）
4. 生成雷达图、趋势图、热力图
5. 可配置的输出路径和数据源
...
OUTPUT_DIR = PROJECT_ROOT / "output" / "topic_analysis"
DATA_PATH = PROJECT_ROOT / "output" / "topic_analysis" / "topic_word_sets.json"
...
class DimensionAnalyzer:
    ...
    def plot_heatmap(self, df, path, title):
        ...
        sns.heatmap(df_transposed, annot=True, fmt=".3f", cmap="viridis")
        plt.title(title)
        plt.xlabel("Era (时期)")
        plt.ylabel("Topic (维度)")
        plt.tight_layout()
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 测试您指定的setting: keywords_era1-法制_era2-[法制+法治]_era3-法治
        target_keywords = {
            'era1': '法制',
            'era2': ['法制', '法治'],
            'era3': '法治'
        ...
        print("\n" + "="*80)
        print("🔧 通用维度分析器演示")
        print("📊 演示setting: era1-法制, era2-[法制+法治], era3-法治")
        print("="*80)
        
        # 演示General Union模式
        print("\n🧪 运行演示分析: General Union + Same Era 归一化")
        analyzer.run_analysis(target_keywords, use_general_union=True, normalize='same_era')
        
        print("\n🎉 演示分析完成！")
        print("💡 提示: 使用专门的分析器进行深入分析：")
