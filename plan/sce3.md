...
本研究采用动态词向量方法刻画语义结构的跨期变化。该方法源自自然语言处理（NLP）的分布式语义传统，是当代大语言模型的基础技术之一：以大规模语料的共现信息学习词语的向量化表示，并以向量之间的接近度表征语义邻近。与依赖个别文本的“快照式”分析不同，词向量能够在长时段与大体量语料上提供结构化、可比较的量化证据。
...
这一向量化框架已在多个人文与社会科学领域被采用，用于识别概念的历史演变与公共话语的框架调整。例如：在社会学与政治传播中，用以追踪政策议题与价值词（如“自由”“平等”“安全”）的相对靠近变化，以揭示议程与框架迁移；在传播学与媒体研究中，用以比较不同媒体或不同时段对同一概念的语义邻域差异；在历史学与数字人文中，用于考察史料中关键政治—法律概念（如“国家”“革命”“宪法”）的语义漂移与语义网络重排。相比之下，法学领域在系统引入动态词向量以研究“法治/法制”等核心术语的语义结构与功能定位方面仍相对稀缺，这也是本文的切入点与方法贡献之一。
...
从方法论角度看，动态词向量依赖大体量且长期连续的文本来保证表示稳定与时间可比。《人民日报》在篇幅规模、年份覆盖与文本一致性方面满足这一要求，从而为词频、直接相似度与基于领域词包的维度得分等多指标提供稳健的数据基础。
...
### 3.4 建模与定义：指标、维度与读图指南

本小节将前述方法转化为可展示、可比较的指标体系。我们的建模遵循“由浅入深”的顺序：先给出最直观的基础指标，用以把握术语替换与直接贴近度的变化；再通过“语义动态指数”揭示“发展/治理”与“约束/程序”两类语义的相对强弱；最后进入由领域知识支持的多维度结构分析，观察“法治/法制”在不同规范价值、实践环节与治理领域上的语义轮廓。

基础层的两个指标分别是“术语频率”与“直接相似度”。前者以年度为单位统计“法制”“法治”的使用频率，观察术语替换的节奏与力度；后者度量“法制—法治”的语义贴近程度，并在时间轴上呈现其收敛或分离的趋势。二者意在回答最直观的问题：术语如何替换、二者彼此是更像还是更不像。需要强调的是，这些指标仅描述“语境靠近”的统计关系，而非价值判断。

解释层的“语义动态指数”在基础相似度之上引入了维度化的对照。我们依据领域知识与专家经验，将与“约束/程序”相关的一组词归为一类，将与“发展/治理”相关的一组词归为另一类；分别计算“法治”（或“法制”）与这两组词的平均贴近度，并以“发展/治理”减去“约束/程序”得到一个方向明确的分值。分值为正，意味着在该时期“法治”相对更靠近“发展/治理”的语境；分值为负，则相对更靠近“约束/程序”的语境。该指数不作规范性评价，旨在以透明方式呈现语境重心的相对位移。

在结构层，我们将“领域词包”推广为系统的维度框架。具体做法是：为若干抽象维度各自构造一份代表性词表，并以目标词与该词表的平均贴近度作为该维度的分值；为确保跨期可比，我们在不同时间段采用一致的词表口径，并在必要时采取“通用并集”的策略以避免因词表漂移带来的人为差异。考虑到术语在中期处于转折状态，我们在关键词设定上采用“兼顾过渡”的口径：早期以“法制”为代表，中期并行使用“法制/法治”（取平均），晚期以“法治”为代表。该设定将在图注中清晰标注。

三组维度的构造与用途如下。其一，内在规范价值维度，用以捕捉“法治”在实质正义、正当程序、权力制衡、权利保障等方面的相对贴近度，从而呈现其与宪政—权利语义的联结强弱。其二，法治实践环节维度，将立法、司法、执法、守法作为四个环节，观察“法治”在制度流程上的指向性差异。其三，治理—发展领域维度，选取经济金融、对外开放、生态环境、科技发展等领域，观察“法治”作为治理工具时与各政策板块的耦合程度。三组维度共同构成从价值、过程到领域的整体视角，使我们能够在“同一坐标系”下比较不同侧面的语义结构。

展示与标准化原则与可比性相关。为便于同一时期内跨维度比较，我们在必要时对分值进行适度的归一化处理，并在图注中明示；跨时期比较时，则以统一的计算口径为基准，避免通过额外变换引入歧义。所有图表均在标题或注释中标注关键词设定与标准化方式，使读者能准确把握比较边界。

读图顺序与解释规则建议如下。首先观察基础层结果：术语频率与直接相似度，建立关于替换与彼此关系的直观印象；其次查看语义动态指数，判断语境重心在“发展/治理”与“约束/程序”之间的相对方向；最后进入结构层的多维度图表，通过热力图与雷达图把握各维度在不同时期的相对强弱，并结合趋势图识别上升或下降的维度。解释时应优先关注变化方向与相对差异，谨慎对待绝对数值大小，并将定量结果与第二节的历史与概念脉络结合阅读。

稳健性与限制在本层同样重要。双时间视角（阶段划分与滑动窗口）为关键结论提供互证；在词表设定上，我们同时报告“各用其表”与“合并词表”两种口径，以检验过渡期设定对结论的影响；并在附录披露低频与未登录词的覆盖情况。需要强调的是，词向量反映的是语境关系而非因果机制，本文据此仅提出描述性与结构性的陈述，避免溢出到规范判断与价值评价。

本小节完成指标与维度的定义，并提供了必要的读图指南。下一节将据此给出实证结果与解释，展示“法治/法制”在不同时期的语义邻近结构、动态指数变化以及在多维度坐标下的结构性差异。

### 3.5 稳健性与局限

为确保结论可信与可解释，本文在方法与指标层面进行了多重稳健性检验，并就研究边界与可能偏差给出明确说明。首先，时间维度上采用阶段性划分与滑动窗口两种视角并行建模：前者强调结构性差异与历史对齐，后者强调连续变化与过渡识别。两种视角在关键趋势与相对强弱上的一致性，是我们判断结论稳健性的首要依据；若二者呈现差异，我们将结合阶段交替的具体语境解释，将其视为“过渡效应”而非矛盾结论。

其次，在词表与口径设置上，我们对比了“各用其表”与“合并词表”两种做法，并在中期阶段采用“法制/法治”并行设定以反映术语转折的现实。多维度分值在同一时期内可进行适度归一化以便横向比较；跨时期比较时坚持统一计算口径，避免因变换引入新的不确定性。图注与附录将同步标注这些口径选择，使读者清楚比较边界与解释前提。

第三，在数据覆盖与质量方面，我们报告未登录词与低频项的覆盖情况，并对极端稀疏导致的异常现象予以提示。预处理阶段对常见虚词的适度过滤与对低频项的约束，旨在降低噪声对相似度估计的影响；同时，我们避免在低覆盖的词项上作强结论，将注意力集中于稳定且具有代表性的语义信号。

第四，在敏感性分析中，我们考察了若干关键设定的小范围变动，包括低频约束、语境范围长度、时间窗口与步长、以及“法治”语义邻域的规模。总体上，本文关注的主线模式对这些改动保持稳定；若个别维度或个别时期对设定较为敏感，我们将在结果叙述中单列说明，并提示读者谨慎解读。

需要强调的局限包括：其一，单一媒体来源固有的体裁与编辑风格可能影响词语分布；其二，官方表达具有策略性，同一术语在不同阶段可能存在“同词异义”；其三，词向量所度量的是语境上的“靠近”，并不能直接推出因果机制或规范性判断，无法据此断定权利保障水平或制度绩效。基于这些理由，本文的定量结论应与第二节的历史与概念脉络相互参照，并与必要的定性分析相结合。

最后，在伦理与复核方面，本文使用公开新闻材料，不涉及个人敏感信息；方法流程与主要设定均在正文与附录中说明，以便学界复核与扩展。至此，第三节完成方法框架与指标体系的交代，下一节将转入实证展示与解释。
